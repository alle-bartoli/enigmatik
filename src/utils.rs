...
"8b,   ,aa  88       88  88  "8a,   ,d88  88      88      88  88,    ,88    88,    88  88`"Yba, 8  
 `"Ybbd8"'  88       88  88   `"YbbdP"Y8  88     8888     88  `"8bbdP"Y8    "Y888  88  88   `Y8aY  
                             aaa,    ,88                                                          
                               "Y8bbdP"                                                           
"#;

    format!("{}\n🔐 Welcome to a secure password generator. \n", logo)
}

// Prompts the user to input their preferences for including symbols and numbers in a password,
// returning a tuple (bool, bool) representing these options.
...
pub fn get_password_length() -> usize {
    loop {
        print!("🔑 Enter your desired password length (min=8, max=100): ");
        ...
        if (8..=100).contains(&length) {
            ...
            println!(
                "⚠️  The password length must be between 8 and 100 characters. Please try again."
            );
        }
    }
}
